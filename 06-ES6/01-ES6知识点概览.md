...
console.log({ a, b, show(){ console.log('a') }}) // { a: 12, b: 5, show: [Function: show] }
```
...
- generator 生成器函数
    ...
    - generator函数前面加一个 `*` 两边可以有空格，靠近函数或`function`
    - 背后实际生成多个小函数，实现走走停停
...
- 数组
    ...
    - for ... in 遍历数组 下标 key
    - for ... of 遍历数组 值 value, 不能用于json，json不支持迭代器
